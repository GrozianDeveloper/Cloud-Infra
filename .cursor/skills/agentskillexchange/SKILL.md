...
Dockge is already a TrueNAS catalog app. Do **not** `docker compose up` another Dockge. Stacks dir / WebUI port: `dockge/README.md`.

`socket.io-client`: `npm install` in `.cursor/skills/truenas-scale/`. Login from that skill’s `.env` (`DOCKGE_USER` / `DOCKGE_PASS`). Do not print creds.

```bash
node scripts/dockge-start.mjs list
node scripts/dockge-start.mjs start <stackName>
node scripts/dockge-start.mjs status <stackName>
node scripts/dockge-start.mjs get <stackName>
node scripts/dockge-start.mjs down <stackName>
```
...
List / image-update: `.cursor/skills/truenas-scale/scripts/dockge-list.mjs` and `dockge-update.mjs`.

`down` / `start` are live stack actions — confirm with the user first.
